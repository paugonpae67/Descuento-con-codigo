...
const checkNotExistDescuento = async (value, { req }) => {
  try {
    ...
    if (restaurant !== null) {
      return Promise.reject(new Error('Thre is a restaurant with this codigo'))
...
const create = [
  ...
  check('descuento').optional().isInt({ min: 0, max: 100 }).toInt(),
  check('codigo').optional().isString().trim().custom(checkNotExistDescuento),
...
const update = [
  ...
  check('userId').not().exists(),
  check('descuento').optional().isInt({ min: 0, max: 100 }).toInt(),
  check('codigo').optional().isString().custom(checkNotExistDescuento),
  check('heroImage').custom((value, { req }) => {
    return checkFileIsImage(req, 'heroImage')
  }).withMessage('Please upload an image with format (jpeg, png).'),
  check('heroImage').custom((value, { req }) => {
